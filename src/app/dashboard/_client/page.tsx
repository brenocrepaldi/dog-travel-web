...
import { Plus, MapPin, ClipboardList, Dog, Star } from 'lucide-react';
	...
	return (
		<div className="space-y-8">
			<PageHeader
				...
				action={
					<Link href="/walks/new" className={cn(buttonVariants({ variant: 'default' }))}>
			...
			{activeWalk ? (
				<Card className="border-primary/40 bg-gradient-to-r from-primary/10 to-primary/5 shadow-sm">
					<CardContent className="flex items-center justify-between p-6">
						...
						<Link href="/walks/1/tracking" className={cn(buttonVariants({ variant: 'default' }))}>
			...
			<div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
				{[
					...
					{ label: 'Minha avaliação', value: '4.9', icon: Star },
				].map((stat) => (
					<Card key={stat.label} interactive className="group cursor-default">
			...
			<section className="space-y-5">
				<div className="flex items-center justify-between">
					...
					<Link href="/walks" className={cn(buttonVariants({ variant: 'ghost', size: 'sm' }))}>
						Ver todos →
					</Link>
				</div>
